 ...
 */
export function generateTimeSeriesData(count) {
 ...
 */
export function generateMultiSeriesData(count, seriesKeys) {
    const start = new Date('2020-01-01').getTime();
    const dayMs = 86_400_000;
    const data = new Array(count);
    const values = Object.fromEntries(seriesKeys.map((k) => [k, 50]));
    for (let i = 0; i < count; i++) {
        const row = { date: new Date(start + i * dayMs) };
 ...
 */
export function generateSeparateSeriesData(count, seriesKeys) {
    const result = {};
    for (const key of seriesKeys) {
 ...
 */
function generatePolygonFeature(centerLon, centerLat, size, vertexCount, id) {
    const coords = [];
 ...
 */
export function generateGeoFeatures(count, verticesPerFeature = 20) {
