...
import { clearCanvasContext, scaleCanvas, getPixelColor, renderPathData, renderText, renderRect, renderCircle, renderEllipse, _createLinearGradient, createLinearGradient, _getComputedStyles, DEFAULT_FILL, } from './canvas.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
function plainFill(fill, opacity = '1') {
    return { styles: { fill, fillOpacity: opacity, opacity, stroke: 'none' } };
}
function plainStroke(stroke, strokeWidth = '1') {
    return {
        styles: {
            fill: 'none',
            stroke,
...
describe('renderText', () => {
    ...
    it('sets textAlign to "center" for textAnchor="middle"', () => {
        renderText(ctx, 'Text', { x: 0, y: 0 }, { styles: { fill: 'black', fillOpacity: '1', opacity: '1', textAnchor: 'middle' } });
        expect(ctx.textAlign).toBe('center');
    });
    it('sets textAlign to "right" for textAnchor="end"', () => {
        renderText(ctx, 'Text', { x: 0, y: 0 }, { styles: { fill: 'black', fillOpacity: '1', opacity: '1', textAnchor: 'end' } });
        expect(ctx.textAlign).toBe('right');
    });
    it('sets textAlign to "left" for textAnchor="start"', () => {
        renderText(ctx, 'Text', { x: 0, y: 0 }, { styles: { fill: 'black', fillOpacity: '1', opacity: '1', textAnchor: 'start' } });
        expect(ctx.textAlign).toBe('left');
    });
    it('sets textAlign from textAlign style when textAnchor is absent', () => {
        renderText(ctx, 'Text', { x: 0, y: 0 }, { styles: { fill: 'black', fillOpacity: '1', opacity: '1', textAlign: 'right' } });
        expect(ctx.textAlign).toBe('right');
    });
...
describe('renderRect', () => {
    ...
    it('clamps rx to half of width', () => {
        ...
        expect(() => renderRect(ctx, { x: 0, y: 0, width: 60, height: 40, rx: 100 }, plainFill('red'))).not.toThrow();
        expect(fillSpy).toHaveBeenCalled();
        ctx.roundRect = originalRoundRect;
    });
});
...
describe('_createLinearGradient', () => {
    ...
    it('calls addColorStop for each provided stop', () => {
        ...
        vi.spyOn(ctx, 'createLinearGradient').mockReturnValue(mockGradient);
...
describe('parseStyleString (via renderPathData style option)', () => {
    ...
    it('handles null style gracefully', () => {
        expect(() => renderPathData(ctx, 'M0,0 L10,0', {
            styles: { fill: 'none', stroke: 'black', strokeOpacity: '1', opacity: '1' },
            style: null,
        })).not.toThrow();
    });
